"""双目鱼眼 + 语义体素 GT 同步采集 (DNG RAW 输出)
====================================================
与 stereo_voxel_capture.py 完全相同的场景/相机/NPC/体素逻辑，
额外输出 12-bit Bayer DNG（通过 rawcam 模块）。

输出目录 output_dng/:
  left/        ← RGB PNG（与原版一致）
  right/       ← RGB PNG
  left_dng/    ← 12-bit Bayer DNG
  right_dng/   ← 12-bit Bayer DNG
  voxel/       ← 语义体素
  meta/        ← 帧元数据

运行：
    isaaclab.bat -p projects/stereo_voxel/scripts/stereo_voxel_capture_dng.py
    isaaclab.bat -p projects/stereo_voxel/scripts/stereo_voxel_capture_dng.py --headless --num_frames 200
    isaaclab.bat -p projects/stereo_voxel/scripts/stereo_voxel_capture_dng.py --no_npc --headless --num_frames 50
"""
...
# rawcam: RGB → 12-bit Bayer RAW → DNG
...
def async_save_dng(path, rgb_array, raw_converter, dng_writer):
...
# Output directories
# ============================================================================
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), "output_dng")
...
ALL_DIRS = [LEFT_DIR, RIGHT_DIR, LEFT_DNG_DIR, RIGHT_DNG_DIR, VOXEL_DIR, META_DIR]
for d in ALL_DIRS:
...
print(f"[Capture] RawCam pipeline initialized: {_sensor_cfg.bit_depth}-bit {_sensor_cfg.bayer_pattern}, noise={_noise_cfg.preset}")
...
def capture_frame(frame_id):
    ...
    async_save_dng(os.path.join(LEFT_DNG_DIR, f"{frame_str}.dng"), rgb_l, _raw_converter, _dng_writer)
    async_save_dng(os.path.join(RIGHT_DNG_DIR, f"{frame_str}.dng"), rgb_r, _raw_converter, _dng_writer)
